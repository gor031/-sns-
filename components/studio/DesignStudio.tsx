...
export const DesignStudio: React.FC<DesignStudioProps> = () => {
  ...
  const handleAIExtract = async (mode: 'bg-remove' | 'object-extract', targetObj?: FabricObject) => {
    const canvas = fabricRef.current;
    if (!canvas) return;
    ...
    try {
      // 1. Get image data from Fabric object
      ...
      const response = await fetch('https://gabjagi031--cardnews-ai-backend-imageprocessor-extract.modal.run', {
        method: 'POST',
        body: formData,
      });

      if (!response.ok) throw new Error('AI 처리 중 오류가 발생했습니다.');
      const result = await response.json();

      // If not initialized, set canvas size to image size
      if (!isInitialized) {
        ...
        // Also need to resize the actual fabric canvas immediately
        canvas.setDimensions({ width: w, height: h });
      }

      // 4. Handle results based on mode
      if (mode === 'bg-remove') {
        const fgImg = await FabricImage.fromURL(result.elements[0].image);
        fgImg.set({
          left: activeObj.left,
          top: activeObj.top,
        ...
        const bgImg = await FabricImage.fromURL(result.background);
        bgImg.set({
          left: activeObj.left,
          top: activeObj.top,
          scaleX: activeObj.scaleX,
      ...
      console.error(err);
      alert('AI 처리에 실패했습니다. 잠시 후 다시 시도해주세요.');
    } finally {
  ...
  }, [canvasWidth, canvasHeight, isInitialized, saveHistory]);

  const handleImageUpload = useCallback((mode?: 'bg-remove') => {
        ...
        <div className="absolute inset-0 z-[100] flex flex-col items-center justify-center bg-slate-50 p-6 overflow-auto">
          <div className="max-w-4xl w-full">
            <div className="text-center mb-12 animate-fade-in-down">
              ...
              <p className="text-lg text-slate-500 font-medium max-w-xl mx-auto">AI 피사체 추출로 이미지를 분석하거나, 자유롭게 빈 캔버스에서 시작해보세요.</p>
            </div>

            <div className="grid md:grid-cols-2 gap-8">
              {/* Start from Photo */}
              <button 
                onClick={() => handleImageUpload('bg-remove')}
              ...
              >
                {isInitialLoading && (
                  <div className="absolute inset-0 z-10 bg-white/80 flex flex-col items-center justify-center">
                    ...
                    <p className="text-sm font-bold text-slate-600">이미지 분석 준비 중...</p>
                  </div>
                )}
                ...
                <h3 className="text-2xl font-black text-slate-800 mb-3">AI 피사체 추출로 시작</h3>
                <p className="text-slate-500 text-sm leading-relaxed mb-8">사진 속 주인공(사람, 상품 등)만 마법처럼 골라내고 나머지는 투명하게 만듭니다. '누끼 따기' 작업에 최적화되어 있습니다.</p>
                
                <div className="mt-auto flex items-center gap-2 text-primary font-bold text-sm">
                  사진 선택하여 시작하기 <Maximize size={16} className="group-hover:translate-x-1 transition-transform" />
                </div>
              </button>
          ...
          <div className="shadow-[0_20px_50px_-12px_rgba(0,0,0,0.15)] hover:shadow-[0_25px_60px_-15px_rgba(0,0,0,0.2)] transition-shadow duration-500 rounded-xl overflow-hidden relative z-10 border border-slate-200/50 bg-white" style={{ width: canvasWidth * zoom, height: canvasHeight * zoom }}>
            ...
            {isProcessingAI && (
              <div className="absolute inset-0 bg-white/60 backdrop-blur-sm flex flex-col items-center justify-center z-50 animate-fade-in">
                ...
                <p className="mt-4 text-sm font-bold text-slate-700">AI가 피사체를 분석 중입니다...</p>
                <p className="text-[10px] text-slate-400 mt-1">최대 10초 정도 소요될 수 있습니다.</p>
              </div>
            )}
          </div>
        </div>
